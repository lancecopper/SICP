...
;;;
(load "/home/lancecopper/code/SICP/ex5.39.scm")
(load "/home/lancecopper/code/SICP/explicit-control-evaluator-adjuvant.scm")
(load "/home/lancecopper/code/SICP/compiler-machine-operation.scm")
...
;;; containing the procedure's parameters, so that the sequence
;;; making up the body is compiled with that extended environment.

;;; At each point in the compilation, compile-variable and
;;; compile-assignment use the compile-time environment
;;; in order to generate the appropriate lexical addresses

(define (compile-lambda-body exp proc-entry ct-env)
  (let ((formals (lambda-parameters exp)))
    (set! ct-env 
        (extend-environment formals vals ct-env)
    (append-instruction-sequences
     (make-instruction-sequence '(env proc argl) '(env)
      `(,proc-entry
        (assign env (op compiled-procedure-env) (reg proc))
