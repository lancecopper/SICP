...
(load "/home/lancecopper/code/SICP/explicit-control-evaluator-adjuvant5.32.scm")
...
(define eceval
  ...
  '(read-eval-print-loop
    ...
    unknown-procedure-type
      ...
      (goto (label signal-error))
    unbound-variable
      (assign val (const unbound-or-unassigned-variable))
      (goto (label signal-error))
    signal-error
      (perform (op user-print) (reg val))
      (goto (label read-eval-print-loop))
    ...
    ev-variable
      ...
      (test (op equal?) (reg val) (const **variable-unbound**))
...
;;; new code added starts
    ev-application-with-symbol-operator
      ...
      (assign continue 
        (label ev-appl-did-operator-without-restore))
    ...
    ev-appl-did-operator
      ...
      (restore env)
    ev-appl-did-operator-without-restore
